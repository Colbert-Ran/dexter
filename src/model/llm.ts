...
export const DEFAULT_PROVIDER = process.env.DEXTER_PROVIDER || "openai";
export const DEFAULT_MODEL = process.env.DEXTER_MODEL || "gpt-5.2";
...
export function getChatModel(
    ...
    const prefix = Object.keys(MODEL_PROVIDERS).find((p) =>
...
export async function callLlm(
    ...
    try {
        result = await withRetry(() =>
            chain.invoke({ prompt }, signal ? { signal } : undefined),
        );
    } catch (e) {
