...
export function getSetting<T>(key: string, defaultValue: T): T {
    ...
    if (key === "provider" && process.env.DEXTER_PROVIDER) {
        return process.env.DEXTER_PROVIDER as unknown as T;
    }
    if (key === "modelId" && process.env.DEXTER_MODEL) {
        return process.env.DEXTER_MODEL as unknown as T;
    }
